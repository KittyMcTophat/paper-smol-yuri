...
func _turn(target_rotation : float, dead_zone : float = 5.0) -> void:
	var current_rotation : float = _sprite_3d.personal_rotation;
	var distance_to_rotate : float = abs(target_rotation - current_rotation);
	...
	if (distance_to_rotate > 180):
		target_rotation -= 360;
		distance_to_rotate = abs(target_rotation - current_rotation);
...
# warning-ignore:return_value_discarded
	_tween_node.interpolate_property(_sprite_3d, "personal_rotation",\
	null, target_rotation, duration, Tween.TRANS_LINEAR);
# warning-ignore:return_value_discarded
	_tween_node.start();
